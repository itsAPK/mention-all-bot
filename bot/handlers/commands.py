...
async def set_all_handler(bot, message):
    if is_group_register(message.chat.username) is True:
        try:
            admins = []
            admin = bot.iter_chat_members(
                message.chat.username, filter='administrators')
            ...
            if message.from_user.id in admins:
                session.query(Group).filter(
                    Group.group == message.chat.username).update({Group.tag_all: True})
                session.commit()
                m = await bot.send_message(message.chat.id, "Sucessfully Set")
                session.add(Message(group=message.chat.username,
...
async def set_admin_only_handler(bot, message):
    if is_group_register(message.chat.username) is True:
        try:
            ...
            if message.from_user.id in admins:
                session.query(Group).filter(
                    Group.group == message.chat.username).update({Group.tag_all: False})
                session.commit()
                m = await bot.send_message(message.chat.id, "Sucessfully Set")
                print(m)
                session.add(Message(group=message.chat.username,
        ...
        try:
            ...
            async for u in admin:
                if u.status == 'creator' or u.status == 'administrators' and u.user.is_bot == False:
                    admins.append(u.user.id)
            if message.from_user.id in admins:
                z = session.query(Message).filter(
                    Message.group == message.chat.username).all()
                for i in z:
                    await bot.delete_messages(message.chat.id, i.message_id)
                    session.query(Message).filter(
                        Message.group == message.chat.username, Message.message_id == i.message_id).delete()
                    session.commit()
                ...
                session.add(
                    Message(group=message.chat.username, message_id=m.id))
                session.commit()
        except Exception as e:
            print(e)
            await bot.send_message(message.chat.id, "Your Group have no permission to use my commands please contact my owner @ronaldo_william")
